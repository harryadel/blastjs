...
import { Match, check } from '../src/match';
...
test('check - check', () => {
  const matches = (value, pattern) => {
    ...
    expect(Match.test(value, pattern)).toBeTruthy();
  };
  ...
  const fails = (value, pattern) => {
    ...
    expect(Match.test(value, pattern)).toBeFalsy();
  };
...
test('check - argument checker', test => {
  ...
  const doesntCheckAllArguments = (f, ...args) => {
    try {
      Match._failIfArgumentsAreNotAllChecked(f, {}, args, 'test');
      console.error({ message: 'expected _failIfArgumentsAreNotAllChecked to throw' });
    } catch (e) {
      expect(e.message).toEqual('Did not check() all arguments during test');
    }
  };
...
test('check - Match error message', () => {
...
test(
  ...
  () => {
    ...
    expect(new Match.Optional()).toEqual(Match.Optional());
    expect(new Match.Maybe()).toEqual(Match.Maybe());
    expect(new Match.OneOf([1])).toEqual(Match.OneOf([1])); // Needs a non-empty array
    expect(new Match.Where()).toEqual(Match.Where());
    expect(new Match.ObjectIncluding()).toEqual(Match.ObjectIncluding());
    expect(new Match.ObjectWithValues()).toEqual(Match.ObjectWithValues());
  }
);
