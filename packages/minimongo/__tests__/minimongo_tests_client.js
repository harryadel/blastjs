...
import {
  hasOwn, _f, MinimongoTest, MinimongoError,
} from '../src/common';
import { LocalCollection } from '../src/local_collection';
import { Minimongo } from '../src/minimongo';
...
const assertOrdering = (test, f, values) => {
  for (let i = 0; i < values.length; i++) {
    let x = f(values[i], values[i]);
    if (x !== 0) {
      throw new Error("value doesn't order as equal to itself");
    }
    if (i + 1 < values.length) {
      ...
      if (!(x < 0)) {
        // XXX super janky
        throw new Error('ordering test failed');
      }
      x = f(more, less);
      if (!(x > 0)) {
        // XXX super janky
...
test('minimongo - basics', () => {
  const c = new LocalCollection();
  let count;

  const fluffyKitten_id = c.insert({ type: 'kitten', name: 'fluffy' });
  ...
  expect(c.find().count()).toEqual(5);
  expect(c.find({ type: 'kitten' }).count()).toEqual(2);
  expect(c.find({ type: 'cryptographer' }).count()).toEqual(3);
  expect(c.find({ type: 'kitten' }).fetch().length).toEqual(2);
  expect(c.find({ type: 'cryptographer' }).fetch().length).toEqual(3);
  expect(fluffyKitten_id).toEqual(c.findOne({ type: 'kitten', name: 'fluffy' })._id);

  c.remove({ name: 'cara' });
  expect(c.find().count()).toEqual(4);
  expect(c.find({ type: 'kitten' }).count()).toEqual(2);
  expect(c.find({ type: 'cryptographer' }).count()).toEqual(2);
  expect(c.find({ type: 'kitten' }).fetch().length).toEqual(2);
  expect(c.find({ type: 'cryptographer' }).fetch().length).toEqual(2);

  count = c.update({ name: 'snookums' }, { $set: { type: 'cryptographer' } });
  expect(count).toEqual(1);
  expect(c.find().count()).toEqual(4);
  expect(c.find({ type: 'kitten' }).count()).toEqual(1);
  expect(c.find({ type: 'cryptographer' }).count()).toEqual(3);
  expect(c.find({ type: 'kitten' }).fetch().length).toEqual(1);
  expect(c.find({ type: 'cryptographer' }).fetch().length).toEqual(3);
  ...
  expect(c.find({ tags: 'flower' }).count()).toEqual(1);
  expect(c.find({ tags: 'fruit' }).count()).toEqual(2);
  expect(c.find({ tags: 'red' }).count()).toEqual(3);
  expect(c.find({ tags: 'flower' }).fetch().length).toEqual(1);
  expect(c.find({ tags: 'fruit' }).fetch().length).toEqual(2);
  expect(c.find({ tags: 'red' }).fetch().length).toEqual(3);

  expect(c.findOne(1).name).toEqual('strawberry');
  expect(c.findOne(2).name).toEqual('apple');
  expect(c.findOne(3).name).toEqual('rose');
  expect(c.findOne(4)).toEqual(undefined);
  expect(c.findOne('abc')).toEqual(undefined);
  expect(c.findOne(undefined)).toEqual(undefined);

  expect(c.find(1).count()).toEqual(1);
  expect(c.find(4).count()).toEqual(0);
  expect(c.find('abc').count()).toEqual(0);
  expect(c.find(undefined).count()).toEqual(0);
  expect(c.find().count()).toEqual(3);
  expect(c.find(1, { skip: 1 }).count(false)).toEqual(1);
  expect(c.find(1, { skip: 1 }).count()).toEqual(0);
  expect(c.find({ _id: 1 }, { skip: 1 }).count(false)).toEqual(1);
  expect(c.find({ _id: 1 }, { skip: 1 }).count()).toEqual(0);
  expect(c.find({ _id: undefined }).count()).toEqual(0);
  expect(c.find({ _id: false }).count()).toEqual(0);
  expect(c.find({ _id: null }).count()).toEqual(0);
  expect(c.find({ _id: '' }).count()).toEqual(0);
  expect(c.find({ _id: 0 }).count()).toEqual(0);
  expect(c.find({}, { skip: 1 }).count(false)).toEqual(3);
  expect(c.find({}, { skip: 1 }).count()).toEqual(2);
  expect(c.find({}, { skip: 2 }).count()).toEqual(1);
  expect(c.find({}, { limit: 2 }).count(false)).toEqual(3);
  expect(c.find({}, { limit: 2 }).count()).toEqual(2);
  expect(c.find({}, { limit: 1 }).count()).toEqual(1);
  expect(c.find({}, { skip: 1, limit: 1 }).count(false)).toEqual(3);
  expect(c.find({}, { skip: 1, limit: 1 }).count()).toEqual(1);
  expect(c.find({ tags: 'fruit' }, { skip: 1 }).count(false)).toEqual(2);
  expect(c.find({ tags: 'fruit' }, { skip: 1 }).count()).toEqual(1);
  expect(c.find({ tags: 'fruit' }, { limit: 1 }).count(false)).toEqual(2);
  expect(c.find({ tags: 'fruit' }, { limit: 1 }).count()).toEqual(1);
  expect(c.find({ tags: 'fruit' }, { skip: 1, limit: 1 }).count(false)).toEqual(2);
  expect(c.find({ tags: 'fruit' }, { skip: 1, limit: 1 }).count()).toEqual(1);
  expect(c.find(1, { sort: ['_id', 'desc'], skip: 1 }).count(false)).toEqual(1);
  expect(c.find(1, { sort: ['_id', 'desc'], skip: 1 }).count()).toEqual(0);
  expect(c.find({ _id: 1 }, { sort: ['_id', 'desc'], skip: 1 }).count(false)).toEqual(1);
  expect(c.find({ _id: 1 }, { sort: ['_id', 'desc'], skip: 1 }).count()).toEqual(0);
  expect(c.find({}, { sort: ['_id', 'desc'], skip: 1 }).count(false)).toEqual(3);
  expect(c.find({}, { sort: ['_id', 'desc'], skip: 1 }).count()).toEqual(2);
  expect(c.find({}, { sort: ['_id', 'desc'], skip: 2 }).count()).toEqual(1);
  expect(c.find({}, { sort: ['_id', 'desc'], limit: 2 }).count(false)).toEqual(3);
  expect(c.find({}, { sort: ['_id', 'desc'], limit: 2 }).count()).toEqual(2);
  expect(c.find({}, { sort: ['_id', 'desc'], limit: 1 }).count()).toEqual(1);
  expect(c.find({}, { sort: ['_id', 'desc'], skip: 1, limit: 1 }).count(false)).toEqual(3);
  expect(c.find({}, { sort: ['_id', 'desc'], skip: 1, limit: 1 }).count()).toEqual(1);
  expect(c.find({ tags: 'fruit' }, { sort: ['_id', 'desc'], skip: 1 }).count(false)).toEqual(2);
  expect(c.find({ tags: 'fruit' }, { sort: ['_id', 'desc'], skip: 1 }).count()).toEqual(1);
  expect(c.find({ tags: 'fruit' }, { sort: ['_id', 'desc'], limit: 1 }).count(false)).toEqual(2);
  expect(c.find({ tags: 'fruit' }, { sort: ['_id', 'desc'], limit: 1 }).count()).toEqual(1);
  expect(c.find({ tags: 'fruit' }, { sort: ['_id', 'desc'], skip: 1, limit: 1 }).count(false)).toEqual(2);
  expect(c.find({ tags: 'fruit' }, { sort: ['_id', 'desc'], skip: 1, limit: 1 }).count()).toEqual(1);

  // Regression test for #455.
  c.insert({ foo: { bar: 'baz' } });
  expect(c.find({ foo: { bam: 'baz' } }).count()).toEqual(0);
  expect(c.find({ foo: { bar: 'baz' } }).count()).toEqual(1);

  // Regression test for #5301
  c.remove({});
  c.insert({ a: 'a', b: 'b' });
  const noop = () => null;
  expect(c.find({ a: noop }).count()).toEqual(1);
  expect(c.find({ a: 'a', b: noop }).count()).toEqual(1);
  expect(c.find({ c: noop }).count()).toEqual(1);
  expect(c.find({ a: noop, c: 'c' }).count()).toEqual(0);
  ...
  expect(after.a).toEqual(1);
  expect(after.b).toEqual(undefined);
  expect(after.c).toEqual(undefined);
  expect(after.d).toEqual(undefined);
});

test('minimongo - error - no options', () => {
  try {
    throw MinimongoError('Not fun to have errors');
  } catch (e) {
    expect(e.message).toEqual('Not fun to have errors');
  }
});

test('minimongo - error - with field', () => {
  try {
    throw MinimongoError('Cats are no fun', { field: 'mice' });
  } catch (e) {
    expect(e.message).toEqual("Cats are no fun for field 'mice'");
  }
});

test('minimongo - cursors', () => {
  const c = new LocalCollection();
  let res;
  ...
  const q = c.find();
  expect(q.count()).toEqual(20);

  // fetch
  res = q.fetch();
  expect(res.length).toEqual(20);
  for (let i = 0; i < 20; i++) {
    expect(res[i].i).toEqual(i);
  }
  // call it again, it still works
  expect(q.fetch().length).toEqual(20);

  // forEach
  let count = 0;
  const context = {};
  q.forEach(function (obj, i, cursor) {
    expect(obj.i).toEqual(count++);
    expect(obj.i).toEqual(i);
    expect(context === this).toBeTruthy();
    expect(cursor === q).toBeTruthy();
  }, context);
  ...
  expect(res.length).toEqual(20);
  for (let i = 0; i < 20; i++) { expect(res[i]).toEqual(i * 2); }
  // call it again, it still works
  expect(q.fetch().length).toEqual(20);

  // findOne (and no rewind first)
  expect(c.findOne({ i: 0 }).i).toEqual(0);
  expect(c.findOne({ i: 1 }).i).toEqual(1);
  const id = c.findOne({ i: 2 })._id;
  expect(c.findOne(id).i).toEqual(2);
});

test('minimongo - transform', () => {
  const c = new LocalCollection();
  c.insert({});
  ...
  expect(
    c.findOne({}, { transform: transformWithoutId })._id,
  ).toEqual(
    c.findOne()._id,
  );
});
...
test('minimongo - misc', () => {
  ...
  let b = EJSON.clone(a);
  expect(a).toEqual(b);
  expect(_f._equal(a, b)).toBeTruthy();
  a.a.push(4);
  expect(b.a.length).toEqual(3);
  a.c = false;
  expect(b.c).toBeTruthy();
  b.d.z = 15;
  a.d.z = 14;
  expect(b.d.z).toEqual(15);
  a.d.y.push(88);
  expect(b.d.y.length).toEqual(1);
  ...
  b = EJSON.clone(a);
  a.x.a = 14;
  expect(b.x.a).toEqual(14); // just to document current behavior
});

test('minimongo - lookup', () => {
  ...
  expect(lookupA0X({ a: [{ x: 1 }] })).toEqual([
  ...
  expect(lookupA0X({ a: [{ x: [1] }] })).toEqual([
    { value: [1], arrayIndices: [0, 'x'] },
    { value: undefined, arrayIndices: [0] }]);
  expect(lookupA0X({ a: 5 })).toEqual([{ value: undefined }]);
  expect(lookupA0X({ a: [{ x: 1 }, { x: [2] }, { y: 3 }] })).toEqual([
    // From interpreting '0' as "0th array element".
    { value: 1, arrayIndices: [0, 'x'] },
    // From interpreting '0' as "after branching in the array, look in the
    // object {x:1} for a field named 0".
  ...
  expect(MinimongoTest.makeLookupFunction('w.x.0.z')({
    w: [{ x: [{ z: 5 }] }],
  })).toEqual([
    // From interpreting '0' as "0th array element".
...
test('minimongo - selector_compiler', () => {
  const matches = (shouldMatch, selector, doc) => {
    ...
    if (doesMatch != shouldMatch) {
      // XXX super janky
      throw new Error(`minimongo match failure: document ${shouldMatch ? "should match, but doesn't"
  ...
  function matchCount(query, count) {
    ...
    if (matches !== count) {
      throw new Error(`minimongo match count failure: matched ${matches} times, but should match ${count} times`);
    }
  }
...
test('minimongo - projection_compiler', () => {
  const testProjection = (projection, tests) => {
    ...
    const equalNonStrict = (a, b, desc) => {
      expect(EJSON.equals(a, b)).toBeTruthy();
    };
...
test('minimongo - fetch with fields', () => {
  const c = new LocalCollection();
  ...
  expect(fetchResults.every((x) => x
    && x.something
    && x.anything
    && x.anything.foo
    && x.anything.foo === 'bar'
    && !hasOwn.call(x, 'nothing')
    && !hasOwn.call(x.anything, 'cool'))).toBeTruthy();
  ...
  expect(fetchResults.length === 25).toBeTruthy();
  ...
  expect(fetchResults.every((x) => x
    && x.something
    && x.i >= 10 && x.i < 20)).toBeTruthy();

  fetchResults.forEach((x, i, arr) => {
    if (!i) return;
    expect(x.i === arr[i - 1].i + 1).toBeTruthy();
  });
...
test('minimongo - fetch with projection, subarrays', () => {
  ...
  testForProjection(
    { 'setA.fieldA': 1, 'setB.anotherB': 1, _id: 0 },
    {
  ...
  testForProjection(
    { 'setA.fieldA': 0, 'setB.anotherA': 0, _id: 0 },
    {
...
test('minimongo - fetch with projection, deep copy', () => {
  ...
  doc.a.x++;
  doc.b.y.z--;
  expect(filteredDoc.a.x).toEqual(42);
  expect(filteredDoc.b.y.z).toEqual(33);
  ...
  doc.a.x = 5;
  expect(filteredDoc.a.x).toEqual(43);
});

test('minimongo - observe ordered with projection', () => {
  ...
  expect(handle.collection === c).toBeTruthy();
  ...
  expect(operations.shift()).toEqual(['changed', { a: 3 }, 0, { a: 2 }]);
  expect(operations.shift()).toEqual(['changed', { a: 11 }, 1, { a: 10 }]);
  c.update({ a: 11 }, { a: 1, b: 44 });
  expect(operations.shift()).toEqual(['changed', { a: 1 }, 1, { a: 11 }]);
  expect(operations.shift()).toEqual(['moved', { a: 1 }, 1, 0, 'foo']);
  c.remove({ a: 2 });
  expect(operations.shift()).toEqual(undefined);
  c.remove({ a: 3 });
  expect(operations.shift()).toEqual(['removed', 'foo', 1, { a: 3 }]);
  ...
  handle = c.find({}, { sort: { a: -1 }, fields: { a: 1 } }).observe(cbs);
  expect(operations.shift()).toEqual(['added', { a: 2 }, 0, null]);
  expect(operations.shift()).toEqual(['added', { a: 1 }, 1, null]);
  handle.stop();
  ...
  expect(operations.shift()).toEqual(['added', { a: 100 }, 0, idA2]);
  handle.stop();

  // test skip and limit.
  c.remove({});
  handle = c.find({}, {
    sort: { a: 1 }, skip: 1, limit: 2, fields: { blacklisted: 0 },
  }).observe(cbs);
  expect(operations.shift()).toEqual(undefined);
  ...
  expect(operations.shift()).toEqual(['added', { a: 3 }, 1, null]);
  c.insert({ a: 4, blacklisted: 6 });
  expect(operations.shift()).toEqual(undefined);
  c.update({ a: 1 }, { a: 0, blacklisted: 4444 });
  expect(operations.shift()).toEqual(undefined);
  c.update({ a: 0 }, { a: 5, blacklisted: 11111 });
  expect(operations.shift()).toEqual(['removed', 'foo', 0, { a: 2 }]);
  expect(operations.shift()).toEqual(['added', { a: 4 }, 1, null]);
  c.update({ a: 3 }, { a: 3.5, blacklisted: 333.4444 });
  expect(operations.shift()).toEqual(['changed', { a: 3.5 }, 0, { a: 3 }]);
  handle.stop();
  ...
  expect(operations.shift()).toEqual(['changed', { a: 2 }, -1, { a: 1 }]);
  c.insert({ a: 10, b: 123.45 });
  expect(operations.shift()).toEqual(['added', { a: 10 }, -1, null]);
  c.update({}, { $inc: { a: 1, b: 2 } }, { multi: true });
  expect(operations.shift()).toEqual(['changed', { a: 3 }, -1, { a: 2 }]);
  expect(operations.shift()).toEqual(['changed', { a: 11 }, -1, { a: 10 }]);
  c.update({ a: 11, b: 125.45 }, { a: 1, b: 444 });
  expect(operations.shift()).toEqual(['changed', { a: 1 }, -1, { a: 11 }]);
  expect(operations.shift()).toEqual(['moved', { a: 1 }, -1, -1, 'foo']);
  c.remove({ a: 2 });
  expect(operations.shift()).toEqual(undefined);
  c.remove({ a: 3 });
  expect(operations.shift()).toEqual(['removed', 'foo', -1, { a: 3 }]);
...
test('minimongo - ordering', () => {
  ...
  const verify = (sorts, docs) => {
    (Array.isArray(sorts) ? sorts : [sorts]).forEach((sort) => {
      ...
      assertOrdering(test, sorter.getComparator(), docs);
    });
  };
  ...
  expect(new Minimongo.Sorter({}).getComparator()({ a: 1 }, { a: 2 })).toEqual(0);
...
test('minimongo - sort', () => {
  ...
  expect(c.find(null, { sort: { b: -1, a: 1 }, limit: 5 }).fetch()).toEqual([]);
  expect(c.find(undefined, { sort: { b: -1, a: 1 }, limit: 5 }).fetch()).toEqual([]);
  expect(c.find(false, { sort: { b: -1, a: 1 }, limit: 5 }).fetch()).toEqual([]);
  ...
  expect(c.find({ a: { $gt: 10 } }, { sort: { b: -1, a: 1 }, skip: 3, limit: 5 }).fetch()).toEqual([
    { a: 14, b: 1, _id: '14_1' },
    { a: 15, b: 1, _id: '15_1' },
    { a: 16, b: 1, _id: '16_1' },
    { a: 17, b: 1, _id: '17_1' },
  ...
  expect(c.find({ a: { $gte: 20 } }, { sort: { a: 1, b: -1 }, skip: 50, limit: 5 }).fetch()).toEqual([
    { a: 45, b: 1, _id: '45_1' },
    { a: 45, b: 0, _id: '45_0' },
    { a: 46, b: 1, _id: '46_1' },
    { a: 46, b: 0, _id: '46_0' },
...
test('minimongo - subkey sort', () => {
  ...
  expect(c.find({}, { sort: { 'a.nope.c': -1 } }).count()).toEqual(6);
});

test('minimongo - array sort', () => {
  const c = new LocalCollection();
  ...
  c.insert({
    up: 1, down: 1, selected: 0, a: { x: [1, 4] },
  });
  c.insert({
    up: 2, down: 2, selected: 1, a: [{ x: [2] }, { x: 3 }],
  });
  c.insert({ up: 0, down: 4, a: { x: 0 } });
  c.insert({
    up: 3, down: 3, selected: 2, a: { x: 2.5 },
  });
  c.insert({
    up: 4, down: 0, selected: 3, a: { x: 5 },
  });
  ...
  const testCursorMatchesField = (cursor, field) => {
    ...
    expect(
      cursor.fetch().map((doc) => doc[field]),
    ).toEqual(
  ...
  testCursorMatchesField(
    c.find({ 'a.x': { $gt: 1 } }, { sort: { 'a.x': 1 } }),
    'selected',
  );
});

test('minimongo - nested array sort', () => {
  const c = new LocalCollection();
  ...
  c.insert({
    ab0x: 0, ab0x_g: 0, g_ab0x: 0, cdx: 0, cdx_cdy: 0, cdy_cdx: 0, n: 0,
  });
  c.insert({
    ab0x: 1, ab0x_g: 2, g_ab0x: 3, cdx: 1, cdx_cdy: 2, cdy_cdx: 4, n: 1, g: 2, c: { d: [{ y: 2 }, { y: 3 }] },
  });
  c.insert({
    ab0x: 2, ab0x_g: 1, g_ab0x: 1, cdx: 2, cdx_cdy: 3, cdy_cdx: 5, n: 2, c: { d: [{ y: 2 }] }, g: 0,
  });
  c.insert({
    ab0x: 3, ab0x_g: 3, g_ab0x: 2, cdx: 6, cdx_cdy: 6, cdy_cdx: 8, n: 3, a: { b: [{ x: 0 }] }, c: { d: [{ x: 1, y: 2 }] }, g: 1,
  });
  c.insert({
    ab0x: 4, ab0x_g: 4, g_ab0x: 4, cdx: 3, cdx_cdy: 1, cdy_cdx: 1, n: 4, a: { b: [{ x: [1, 4] }] }, c: { d: [] }, g: 2,
  });
  c.insert({
    ab0x: 5, ab0x_g: 5, g_ab0x: 5, cdx: 7, cdx_cdy: 7, cdy_cdx: 3, n: 5, a: { b: [{ x: [2] }, { x: 3 }] }, c: { d: [{ x: 2, y: 2 }, { x: 3, y: 1 }] }, g: 3,
  });
  c.insert({
    ab0x: 6, ab0x_g: 6, g_ab0x: 6, cdx: 8, cdx_cdy: 8, cdy_cdx: 2, n: 6, a: { b: [{ x: 2.5 }] }, c: { d: [{ x: 2, y: 2 }, { x: 3 }] }, g: 4,
  });
  c.insert({
    ab0x: 7, ab0x_g: 7, g_ab0x: 7, cdx: 4, cdx_cdy: 4, cdy_cdx: 6, n: 7, a: { b: [{ x: 5 }] }, c: { d: [{ y: 2 }, { y: 3 }] }, g: 5,
  });
  c.insert({
    ab0x: 8, ab0x_g: 8, g_ab0x: 8, cdx: 5, cdx_cdy: 5, cdy_cdx: 7, n: 8, a: { b: [{ x: 6 }, { x: 7 }] }, c: { d: [{ y: 2 }, { x: 1.5, y: 2 }] }, g: 6,
  });
...
test('minimongo - sort function', () => {
  ...
  expect(c.find({}, { sort: sortFunction }).fetch()).toEqual(c.find({}).fetch().sort(sortFunction));
  expect(c.find({}).fetch()).not.toEqual(c.find({}).fetch().sort(sortFunction));
  expect(c.find({}, { sort: { a: -1 } }).fetch()).toEqual(c.find({}).fetch().sort(sortFunction));
});

test('minimongo - binary search', () => {
  ...
  const checkSearch = (cmp, array, value, expected, message) => {
    ...
    if (expected != actual) {
      throw new Error(`${message} : Expected index ${expected} but had ${actual}`);
    }
  };
...
test('minimongo - modify', () => {
  ...
  const upsert = (query, mod, expected) => {
    ...
    if (expected._id) {
      expect(result.insertedId).toEqual(expected._id);
    } else {
      delete actual._id;
    }
  ...
  // $currentDate
  modify({}, { $currentDate: { a: true } }, (result, msg) => { expect(result.a).toBeInstanceOf(Date); });
  modify({}, { $currentDate: { a: { $type: 'date' } } }, (result, msg) => { expect(result.a).toBeInstanceOf(Date); });
  ...
  modify(
    { a: [1, 2] },
    { $push: { a: { $each: [0], $position: 0 } } },
    { a: [0, 1, 2] },
  );
  modify(
    { a: [1, 2] },
    { $push: { a: { $each: [-1, 0], $position: 0 } } },
    { a: [-1, 0, 1, 2] },
  );
  modify({ a: [1, 3] }, { $push: { a: { $each: [2], $position: 1 } } }, { a: [1, 2, 3] });
  modify(
    { a: [1, 4] },
    { $push: { a: { $each: [2, 3], $position: 1 } } },
    { a: [1, 2, 3, 4] },
  );
  modify({ a: [1, 2] }, { $push: { a: { $each: [3], $position: 3 } } }, { a: [1, 2, 3] });
  modify(
    { a: [1, 2] },
    { $push: { a: { $each: [3], $position: 99 } } },
    { a: [1, 2, 3] },
  );
  modify(
    { a: [1, 2] },
    { $push: { a: { $each: [3], $position: 99, $slice: -2 } } },
    { a: [2, 3] },
  );
  modify(
    { a: [{ x: 1 }, { x: 2 }] },
    {
      $push: {
        a: {
          $each: [{ x: 3 }], $position: 0, $sort: { x: 1 }, $slice: -3,
        },
      },
    },
    { a: [{ x: 1 }, { x: 2 }, { x: 3 }] },
  ...
  modify(
    { a: [{ x: 1 }, { x: 2 }] },
    {
      $push: {
        a: {
          $each: [{ x: 3 }], $position: 0, $sort: { x: 1 }, $slice: 0,
        },
      },
    },
    { a: [] },
...
test('minimongo - observe ordered', () => {
  const operations = [];
  const cbs = log_callbacks(operations);
  let handle;
  ...
  // test _suppress_initial
  handle = c.find({}, { sort: { a: -1 } }).observe({ _suppress_initial: true, ...cbs });
  expect(operations.shift()).toEqual(undefined);
  c.insert({ a: 100 });
  expect(operations.shift()).toEqual(['added', { a: 100 }, 0, idA2]);
  handle.stop();
  ...
  expect(operations.shift()).toEqual(['added', { a: 1 }, 0, null]);
  expect(operations.shift()).toEqual(['added', { a: 2 }, 1, null]);
  expect(operations.shift()).toEqual(undefined);
  c.remove({ a: 2 });
  expect(operations.shift()).toEqual(['removed', 'two', 1, { a: 2 }]);
  expect(operations.shift()).toEqual(['added', { a: 3 }, 1, null]);
  expect(operations.shift()).toEqual(undefined);
  handle.stop();
...
[true, false].forEach((ordered) => {
  test(`minimongo - observe ordered: ${ordered}`, () => {
    const c = new LocalCollection();
...
test('minimongo - saveOriginals', () => {
  ...
  expect(originals.size()).toEqual(affected.length);
  affected.forEach((id) => {
    expect(originals.has(id)).toBeTruthy();
  });
  expect(originals.get('bar')).toEqual({ _id: 'bar', x: 'updateme' });
  expect(originals.get('baz')).toEqual({ _id: 'baz', x: 'updateme' });
  expect(originals.get('quux')).toEqual({ _id: 'quux', y: 'removeme' });
  expect(originals.get('whoa')).toEqual({ _id: 'whoa', y: 'removeme' });
  expect(originals.get('hooray')).toEqual(undefined);

  // Verify that changes actually occured.
  expect(c.find().count()).toEqual(4);
  expect(c.findOne('foo')).toEqual({ _id: 'foo', x: 'untouched' });
  expect(c.findOne('bar')).toEqual({
    _id: 'bar', x: 'updateme', z: 5, k: 7,
  });
  expect(c.findOne('baz')).toEqual({ _id: 'baz', x: 'updateme', z: 5 });
  expect(c.findOne('hooray')).toEqual({ _id: 'hooray', z: 'insertme' });

  // The next call doesn't get the same originals again.
  c.saveOriginals();
  originals = c.retrieveOriginals();
  expect(originals).toBeTruthy();
  expect(originals.empty()).toBeTruthy();
  ...
  c.remove('temp');
  originals = c.retrieveOriginals();
  expect(originals.size()).toEqual(1);
  expect(originals.has('temp')).toBeTruthy();
  expect(originals.get('temp')).toEqual(undefined);
});

test('minimongo - saveOriginals errors', () => {
  const c = new LocalCollection();
  // Can't call retrieve before save.
  expect(() => { c.retrieveOriginals(); }).toThrow();
  c.saveOriginals();
  // Can't call save twice.
  expect(() => { c.saveOriginals(); }).toThrow();
});

test('minimongo - objectid transformation', () => {
  const testId = (item) => {
    expect(item).toEqual(MongoID.idParse(MongoID.idStringify(item)));
  };
  ...
  expect('ffffffffffff').toEqual(MongoID.idParse(MongoID.idStringify('ffffffffffff')));
});

test('minimongo - objectid', () => {
  const randomOid = new MongoID.ObjectID();
  const anotherRandomOid = new MongoID.ObjectID();
  expect(randomOid).not.toEqual(anotherRandomOid);
  expect(() => { new MongoID.ObjectID('qqqqqqqqqqqqqqqqqqqqqqqq'); }).toThrow();
  expect(() => { new MongoID.ObjectID('ABCDEF'); }).toThrow();
  expect(randomOid).toEqual(new MongoID.ObjectID(randomOid.valueOf()));
});

test('minimongo - pause', () => {
  const operations = [];
  const cbs = log_callbacks(operations);
  ...
  c.resumeObservers();
  expect(operations.shift()).toEqual(['changed', { a: 3 }, 0, { a: 1 }]);
  expect(operations.length).toEqual(0);

  // test special case for remove({})
  c.pauseObservers();
  expect(c.remove({})).toEqual(1);
  expect(operations.length).toEqual(0);
  c.resumeObservers();
  expect(operations.shift()).toEqual(['removed', 1, 0, { a: 3 }]);
  expect(operations.length).toEqual(0);

  h.stop();
});

test('minimongo - ids matched by selector', () => {
  const check = (selector, ids) => {
    ...
    expect(idsFromSelector).toEqual(ids);
  };
  check('foo', ['foo']);
  check({ _id: 'foo' }, ['foo']);
...
test('minimongo - reactive stop', () => {
  ...
  expect(y).toEqual('ABC');

  sortOrder.set(-1);
  expect(x).toEqual('ABC');
  expect(y).toEqual('ABC');
  Tracker.flush();
  expect(x).toEqual('CBA');
  expect(y).toEqual('CBA');

  coll.insert({ _id: 'D' });
  coll.insert({ _id: 'E' });
  expect(x).toEqual('EDCBA');
  expect(y).toEqual('EDCBA');
...
test('minimongo - immediate invalidate', () => {
  const coll = new LocalCollection();
  coll.insert({ _id: 'A' });
...
test('minimongo - count on cursor with limit', () => {
  const coll = new LocalCollection();
  let count; let
    unlimitedCount;
  ...
  expect(unlimitedCount).toEqual(4);

  coll.remove('A'); // still 3 in the collection
  Tracker.flush();
  expect(count).toEqual(3);
  expect(unlimitedCount).toEqual(3);

  coll.remove('B'); // expect count now 2
  Tracker.flush();
  expect(count).toEqual(2);
  expect(unlimitedCount).toEqual(2);
  ...
  Tracker.flush();
  expect(count).toEqual(3);
  expect(unlimitedCount).toEqual(4); // unlimitedCount should be 4 now

  c.stop();
});

test('minimongo - reactive count with cached cursor', () => {
  const coll = new LocalCollection();
  const cursor = coll.find({});
  let firstAutorunCount; let
    secondAutorunCount;
  Tracker.autorun(() => {
  ...
  expect(firstAutorunCount).toEqual(0);
  expect(secondAutorunCount).toEqual(0);
  coll.insert({ i: 1 });
  coll.insert({ i: 2 });
  coll.insert({ i: 3 });
  Tracker.flush();
  expect(firstAutorunCount).toEqual(3);
  expect(secondAutorunCount).toEqual(3);
});

test('minimongo - $near operator tests', () => {
  let coll = new LocalCollection();
  coll.insert({ rest: { loc: [2, 3] } });
  coll.insert({ rest: { loc: [-3, 3] } });
  coll.insert({ rest: { loc: [5, 5] } });

  expect(coll.find({ 'rest.loc': { $near: [0, 0], $maxDistance: 30 } }).count()).toEqual(3);
  expect(coll.find({ 'rest.loc': { $near: [0, 0], $maxDistance: 4 } }).count()).toEqual(1);
  const points = coll.find({ 'rest.loc': { $near: [0, 0], $maxDistance: 6 } }).fetch();
  points.forEach((point, i, points) => {
    expect(!i || distance([0, 0], point.rest.loc) >= distance([0, 0], points[i - 1].rest.loc)).toBeTruthy();
  });
  ...
  const data = [{
    category: 'BURGLARY', descript: 'BURGLARY OF STORE, FORCIBLE ENTRY', address: '100 Block of 10TH ST', location: { type: 'Point', coordinates: [-122.415449723856, 37.7749518087273] },
  },
  {
  ...
  expect(close15.length).toEqual(1);
  expect(close15[0].descript).toEqual('GRAND THEFT OF PROPERTY');
  ...
  expect(close20.length).toEqual(4);
  expect(close20[0].descript).toEqual('GRAND THEFT OF PROPERTY');
  expect(close20[1].descript).toEqual('PETTY THEFT FROM LOCKED AUTO');
  expect(close20[2].descript).toEqual('POSSESSION OF BURGLARY TOOLS');
  expect(close20[3].descript).toEqual('POSS OF PROHIBITED WEAPON');
  ...
  expect(operations.shift()).toEqual(['added', { k: 9, a: { b: [5, 5] } }, 0, null]);
  expect(
    operations.shift(),
  ).toEqual(
  ...
  expect(operations.shift()).toEqual(['added', { a: { b: [3, 3] } }, 1, 'x']);

  handle.stop();
});

// issue #2077
test('minimongo - $near and $geometry for legacy coordinates', () => {
  const coll = new LocalCollection();
  ...
  expect(coll.find({ loc: { $near: [0, 0], $maxDistance: 4 } }).count()).toEqual(2);
  expect(coll.find({ loc: { $near: { $geometry: { type: 'Point', coordinates: [0, 0] } } } }).count()).toEqual(4);
  expect(coll.find({ loc: { $near: { $geometry: { type: 'Point', coordinates: [0, 0] }, $maxDistance: 200000 } } }).count()).toEqual(2);
});

// Regression test for #4377. Previously, "replace" updates didn't clone the
// argument.
test('minimongo - update should clone', () => {
  const x = [];
  const coll = new LocalCollection();
  const id = coll.insert({});
  coll.update(id, { x });
  x.push(1);
  expect(coll.findOne(id)).toEqual({ _id: id, x: [] });
});

// See #2275.
test('minimongo - fetch in observe', () => {
  const coll = new LocalCollection();
  let callbackInvoked = false;
  const observe = coll.find().observeChanges({
    added(id, fields) {
      callbackInvoked = true;
      expect(fields).toEqual({ foo: 1 });
...
// See #2254
test('minimongo - fine-grained reactivity of observe with fields projection', () => {
...
test('minimongo - fine-grained reactivity of query with fields projection', () => {
  const X = new LocalCollection();
  const id = 'asdf';
  X.insert({ _id: id, foo: { bar: 123 } });
...
test('minimongo - reactive skip/limit count while updating', () => {
  const X = new LocalCollection();
  let count = -1;
  ...
  expect(count).toEqual(0);
  expect(unlimitedCount).toEqual(0);

  X.insert({});
  Tracker.flush({ _throwFirstError: true });
  expect(count).toEqual(0);
  expect(unlimitedCount).toEqual(1);
...
test('minimongo - cannot insert using invalid field names', () => {
  const collection = new LocalCollection();
...
test('minimongo - cannot $set with null bytes', () => {
  ...
  }).toThrowError('Key \0a must not contain null bytes');
});

// Makes sure $rename's cannot be performed using null bytes
// https://docs.mongodb.com/manual/reference/limits/#Restrictions-on-Field-Names
test('minimongo - cannot $rename with null bytes', () => {
  const collection = new LocalCollection();
  ...
  }).toThrowError("The 'to' field for $rename cannot contain an embedded null byte");
});
