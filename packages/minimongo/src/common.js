...
import { IdMap } from '@blastjs/id-map';
import { MongoID } from '@blastjs/mongo-id';
import { Decimal } from '@blastjs/mongo-decimal';
import { Tracker } from '@blastjs/tracker';
import { gju } from '@blastjs/geojson-utils';
...
export function _selectorIsId(selector) {
...
export function _selectorIsIdPerhapsAsObject(selector) {
...
export function _checkSupportedProjection(fields) {
  ...
  Object.keys(fields).forEach((keyPath) => {
    ...
    if (typeof value === 'object'
        && ['$elemMatch', '$meta', '$slice'].some((key) => hasOwn.call(value, key))) {
      throw MinimongoError(
...
export function _isPlainObject(x) { return x && _f._type(x) === 3; }
...
export const _wrapTransform = (transform) => {
  if (!transform) {
    return null;
  }
...
export const _CachingChangeObserver = class _CachingChangeObserver {
  constructor(options = {}) {
