...
const makeObjectLiteral = function (obj) {
  const parts = [];
  for (const k in obj) { parts.push(`${BlastTools.toObjectLiteralKey(k)}: ${obj[k]}`); }
  return `{${parts.join(', ')}}`;
};
...
Object.assign(CodeGen.prototype, {
  codeGenTemplateTag(tag) {
    ...
    if (tag.position === HTMLTools.TEMPLATE_TAG_POSITION.IN_START_TAG) {
      ...
      return BlastTools.EmitCode(`function () { return ${
      ...
      return BlastTools.EmitCode(code);
      ...
      if (tag.type === 'BLOCKOPEN'
        ...
        if (path[0] === 'each' && args.length >= 2 && args[1][0] === 'PATH'
          ...
          }, _variable: ${BlastTools.toJSLiteral(variable)} }; }`;
        } else if (path[0] === 'let') {
          const dataProps = {};
          args.forEach((arg) => {
            if (arg.length !== 3) {
            ...
            const argKey = arg[2];
            dataProps[argKey] = `function () { return Spacebars.call(${
      ...
      return BlastTools.EmitCode(includeCode);
    } if (tag.type === 'ESCAPE') {
      return tag.value;
    }
  ...
  codeGenPath(path, opts) {
    ...
    const firstPathItem = BlastTools.toJSLiteral(path[0]);
    ...
    if (path.length > 1) {
      code = `Spacebars.dot(${code}, ${
        path.slice(1).map(BlastTools.toJSLiteral).join(', ')})`;
    }
  ...
  codeGenArgValue(arg) {
    ...
    switch (argType) {
      ...
      case 'BOOLEAN':
      case 'NULL':
        argCode = BlastTools.toJSLiteral(argValue);
        break;
      case 'PATH':
