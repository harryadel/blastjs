export default function (html) {
  ...
  h = h.replace(/<(\w+)\s+(.*?)\s*>/g, (m, tagName, attrs) => {
    ...
    attrs = attrs.replace(/\$blast_teardown_callbacks="[^"]*"/g, ' ');
    // And by DOMRange to keep track of the element's DOMRange
    attrs = attrs.replace(/\$blast_range="[^"]*"/g, ' ');
