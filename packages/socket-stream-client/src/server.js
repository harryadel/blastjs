...
export class ClientStream extends StreamClientCommon {
  ...
  _getProxyUrl(targetUrl) {
    ...
    let proxy = process.env.HTTP_PROXY || process.env.http_proxy || null;
  ...
  _launchConnection() {
    ...
    const targetUrl = toWebsocketUrl(this.endpoint);
    let fayeOptions = {
      headers: this.headers,
      extensions: [deflate],
    };
    fayeOptions = Object.assign(fayeOptions, this.npmFayeOptions);
    const proxyUrl = this._getProxyUrl(targetUrl);
    if (proxyUrl) {
    ...
    const subprotocols = [];
    const client = (this.client = new FayeWebSocket.Client(
      targetUrl,
      subprotocols,
      fayeOptions,
    ));
    ...
    const clientOnIfCurrent = (event, description, callback) => {
      this.client.on(
        event,
        (...args) => {
    ...
    clientOnIfCurrent('error', 'stream error callback', (error) => {
      if (!this.options._dontPrintErrors) { console.error('stream error', error.message); }
