...
export function isNully(node) {
  ...
  if (isArray(node)) {
    ...
    for (let i = 0; i < node.length; i++) { if (!isNully(node[i])) return false; }
    return true;
  }
...
export function flattenAttributes(attrs) {
  ...
  for (let i = 0, N = (isList ? attrs.length : 1); i < N; i++) {
    ...
    if ((typeof oneAttrs !== 'object')
        || isConstructedObject(oneAttrs)) { throw new Error(`Expected plain JS object as attrs, found: ${oneAttrs}`); }
