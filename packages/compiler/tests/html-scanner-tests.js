const TemplatingTools = require('../src/templating-tools/templating-tools.js');
Tinytest.add('templating-tools - html scanner', function (test) {
  var testInString = function (actualStr, wantedContents) {
    if (actualStr.indexOf(wantedContents) >= 0) test.ok();
    else
      test.fail(
        'Expected ' +
          JSON.stringify(wantedContents) +
          ' in ' +
          JSON.stringify(actualStr)
      );
  };
  ...
  var checkError = function (f, msgText, lineNum) {
    try {
      ...
      if (e.line === lineNum) test.ok();
      else
        test.fail(
          'Error should have been on line ' + lineNum + ', not ' + e.line
        );
  ...
  var simpleTemplate = function (templateName, content) {
    ...
    var viewName =
      templateName.slice(0, 1) + 'Template.' + templateName.slice(1);
  ...
  checkResults(
    scanForTest(
      '<template foo = "bar" name  ="favoritefood" baz= "qux"  >' +
        'pizza</template\n\n>'
    ),
    simpleTemplate('"favoritefood"', '"pizza"')
  );
  ...
  checkError(
    function () {
      return scanForTest('<!doctype html>');
    },
    'DOCTYPE',
    1
  );
  ...
  checkError(
    function () {
      return scanForTest('<template name="foo\'>' + 'pizza</template>');
    },
    'error in tag',
    1
  );
